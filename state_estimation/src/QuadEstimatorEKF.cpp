...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  auto quat = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  quat.IntegrateBodyRate(gyro, dtIMU);
  const float predPitch = quat.Pitch(), predRoll = quat.Roll();
  ekfState(6) = quat.Yaw();
  ...
  rollEst = attitudeTau / (attitudeTau + dtIMU) * (predRoll)+dtIMU / (attitudeTau + dtIMU) * accelRoll;
  pitchEst = attitudeTau / (attitudeTau + dtIMU) * (predPitch)+dtIMU / (attitudeTau + dtIMU) * accelPitch;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  VectorXf StatePrediction = curState;
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  StatePrediction(0) += curState(3)*dt;
  StatePrediction(1) += curState(4)*dt;
  StatePrediction(2) += curState(5)*dt;
  const V3F temp = V3F(0.0F, 0.0F, static_cast<float>(CONST_GRAVITY));
  const V3F InertialAcceleration = attitude.Rotate_BtoI(accel) - temp;
  StatePrediction(3) += InertialAcceleration[0]*dt;
  StatePrediction(4) += InertialAcceleration[1]*dt;
  StatePrediction(5) += InertialAcceleration[2]*dt;
  StatePrediction(6) = curState(6);
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  const float phi = roll, theta = pitch, si = yaw;
  const float cosPhi = std::cos(phi), sinPhi = std::sin(phi);
  const float Ctheta = std::cos(theta), Stheta = std::sin(theta);
  const float cosSi = std::cos(si), sinSi = std::sin(si);
  RbgPrime(2,0) = 0.0F;
  RbgPrime(2,1) = 0.0F;
  RbgPrime(2,2) = 0.0F;
  RbgPrime(0,0) = -Ctheta*sinSi;
  RbgPrime(0,1) = -sinPhi*Stheta*sinSi - Ctheta*cosSi;
  RbgPrime(1,1) = sinPhi*Stheta*cosSi - cosPhi*sinSi;
  RbgPrime(1,2) = cosPhi*Stheta*cosSi + sinPhi*sinSi;
  RbgPrime(0,2) = -cosPhi*Stheta*sinSi + sinPhi*cosSi;
  RbgPrime(1,0) = Ctheta*cosSi;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  acc(1) = accel[1];
  acc(2) = accel[2];
  const VectorXf var = RbgPrime * acc;
  // G Diff cal
  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  ...
  gPrime(4, 6) = var(1)*dt;
  gPrime(5, 6) = var(2)*dt;
  // Cov matrix plus Noise Covarianc Added 
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  for (auto itr = 0U; itr < 6U; itr++)
  {
    hPrime(itr, itr) = 1.0F;
    zFromX(itr) = ekfState(itr);
  }
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  hPrime(6) = 1.0F;
  zFromX(0) = ekfState(6);
  const float diff = normAng(z(0) - zFromX(0));
  zFromX(0) = z(0) - diff;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
