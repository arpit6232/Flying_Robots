...
class BackyardFlyer(Drone):
    ...
    def local_position_callback(self):
        ...
        if self.flight_state == States.TAKEOFF:
            ...
            if np.linalg.norm(self.local_position - self.target_position) <= 0.75:
                print(self.local_position, self.target_position)
                if len(self.all_waypoints) != 0:
                    self.waypoint_transition()
                elif np.linalg.norm(self.local_velocity) < 0.1:
    ...
    def calculate_box(self):
        ...
        """
        sq_size = 10
        wp1 = np.array([sq_size, 0, -3])
        wp2 = np.array([sq_size, sq_size, -3])
        wp3 = np.array([0, sq_size, -3])
        wp4 = np.array([0, 0, -3])
    ...
    def waypoint_transition(self):
        ...
        print("heading from {0} to {1}".format(self.local_position, self.target_position))
